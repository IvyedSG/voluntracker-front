<!-- filepath: d:\Diego\Escritorio\Voluntracker\voluntracker-front\components\coordinadores\CoordinadorGrid.vue -->
<template>
  <div
  ...
  >
    ...
    >
      ...
      <div class="flex justify-between">
        ...
        <div class="flex space-x-3">
          ...
          <div>
            <h3 class="text-white font-medium leading-tight">
              {{ coordinador.nombre }}
            </h3>
      ...
      <div class="mt-3 pt-3 border-t border-gray-800/70">
        <div class="flex items-center justify-between">
          <UBadge
            ...
            size="md"
            :icon="
              coordinador.nivelAcceso === 'avanzado'
                ? 'i-heroicons-star'
          ...
          <span class="text-xs text-gray-500"
            >{{ coordinador.areasAsignadas.length }} áreas</span
          >
        </div>
      ...
      <div class="mt-3 text-sm text-gray-400">
        ...
        <div v-if="coordinador.ultimoAcceso" class="flex items-center mt-1">
          <UIcon
            ...
            class="mr-1 text-blue-500/70"
          />
          <span
            >Último acceso:
            {{ formatLastAccess(coordinador.ultimoAcceso) }}</span
          >
        </div>
      </div>
    ...
    <div
      v-if="coordinadores.length === 0"
      class="col-span-full p-10 text-center text-gray-400 bg-gray-900/30 rounded-xl border border-gray-800 shadow-inner"
    >
      <UIcon
        name="i-heroicons-shield-check"
        class="h-12 w-12 mx-auto mb-3 text-gray-600 opacity-50"
      />
      <p class="text-lg">No hay coordinadores registrados</p>
      <UButton
        class="mt-4 shadow-md hover:shadow-lg hover:shadow-blue-900/20 transition-all"
        @click="$emit('add')"
      >
        Añadir coordinador
      </UButton>
...
defineEmits<{
  (
    e: "view" | "edit" | "toggle-status" | "delete" | "invite",
...
// Formateadores
// ✅ CORREGIDO: Función que coincide con los datos reales
function formatRol(rol: string): string {
  switch (rol) {
    ...
    case "coordinador":
      return "Coordinador";
    // ✅ Mantener compatibilidad con valores antiguos si existen
    case "admin_global":
      return "Administrador Global";
    case "lider_local":
      return "Líder Local";
    case "coordinador_local":
      return "Coordinador Local";
    case "voluntario":
      return "Voluntario";
    default:
      return rol;
...
// ✅ CORREGIDO: Colores que existen en Nuxt UI
function getEstadoColor(estado: string): string {
  switch (estado) {
    case "activo":
      return "green"; // ❌ Era "success"
    case "inactivo":
      return "orange"; // ❌ Era "warning"
    case "pendiente":
      return "blue"; // ❌ Era "info"
    default:
      return "gray"; // ❌ Era "neutral"
  }
}
