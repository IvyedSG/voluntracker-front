...
<template>
  <!-- Eliminar UModal y usar estructura HTML personalizada -->
      ...
      >
        <div class="p-5">
          <div class="flex items-center mb-6">
            <div
              class="w-12 h-12 rounded-full bg-yellow-900/30 flex items-center justify-center mr-4"
            >
              <UIcon
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="space-y-4">
              ...
              <UFormGroup label="Nombre completo" name="nombre" required>
                ...
                />
                <template #error>
                  <p v-if="errors.nombre" class="text-red-400 text-sm mt-1">
                    {{ errors.nombre }}
                  </p>
                </template>
              </UFormGroup>
              ...
              <UFormGroup label="Correo electrónico" name="correo" required>
                ...
                />
                <template #error>
                  <p v-if="errors.correo" class="text-red-400 text-sm mt-1">
                    {{ errors.correo }}
                  </p>
                </template>
                <template #description>
                  <p class="text-gray-400 text-xs">
                    Cambiar el correo electrónico requerirá reenviar la
                    invitación al nuevo correo.
                  </p>
                </template>
              </UFormGroup>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="space-y-4">
                  ...
                  >
                    <template #option="{ option }">
                      <div class="flex items-center">
                        <UIcon
                          :name="getEstadoIcon(option.value)"
                          class="mr-2"
                          :class="getEstadoIconClass(option.value)"
                        />
                ...
                <UFormGroup label="Rol" name="rol" required>
                  ...
                  <template #error>
                    <p v-if="errors.rol" class="text-red-400 text-sm mt-1">
                      {{ errors.rol }}
                    </p>
                  </template>
                </UFormGroup>
                ...
                <UFormGroup label="Nivel de acceso" name="nivel" required>
                  ...
                  <template #error>
                    <p v-if="errors.nivel" class="text-red-400 text-sm mt-1">
                      {{ errors.nivel }}
                    </p>
                  </template>
                </UFormGroup>
              ...
              <div>
                <UFormGroup label="Áreas asignadas" name="areas">
                  ...
                  >
                    <template #label>
                      <div class="flex items-center">
                        <span v-if="form.areas.length === 0"
                          >Seleccionar áreas</span
                        >
                        <span v-else
                          >{{ form.areas.length }} áreas seleccionadas</span
                        >
                      </div>
                    </template>
                  </USelectMenu>
                  <template #description>
                    <p class="text-gray-400 text-xs">
                      Las áreas determinan a qué partes del sistema tendrá
                      acceso este coordinador.
                    </p>
                  </template>
                </UFormGroup>
                ...
                <div
                  class="mt-5 p-4 bg-gray-800/30 rounded-lg border border-gray-700/50"
                >
                  ...
                  <div class="grid grid-cols-1 gap-2 text-xs text-gray-400">
                    <div class="flex justify-between">
                      ...
                      <span class="font-medium text-gray-300">{{
                        formatDate(coordinador.fechaRegistro)
                      }}</span>
                    </div>
                    <div class="flex justify-between">
                      ...
                      <span class="font-medium text-gray-300">
                        {{
                          coordinador.ultimoAcceso
                            ? formatDate(coordinador.ultimoAcceso)
                            : "Nunca"
                        }}
                      </span>
                    </div>
...
function initializeForm() {
  if (props.coordinador) {
    form.value = {
      ...
      areas: props.coordinador.areasAsignadas
        ? props.coordinador.areasAsignadas.map((a) => a.nombre)
        : [], // Corregido y mapeado
...
function getEstadoColor(estado) {
  switch (estado) {
    case "activo":
      return "green";
    case "inactivo":
      return "orange"; // Cambio de 'yellow' a 'orange'
    case "pendiente":
      return "blue";
    default:
      return "gray";
...
// También bloquear scroll del body:
watch(
  () => props.modelValue,
  (isOpen) => {
    if (isOpen) {
